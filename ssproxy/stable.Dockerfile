...
ENV SHADOWSOCKS_URL https://github.com/shadowsocks/shadowsocks-libev/releases/download/v$SHADOWSOCKS_VER/shadowsocks-libev-$SHADOWSOCKS_VER.tar.gz
...
RUN yum install git vim wget net-tools gcc automake make pcre-devel \
    ...
    #  下载并安装shadowsocks
    && wget $SHADOWSOCKS_URL -O ./shadowsocks-libev-$SHADOWSOCKS_VER.tar.gz \
    && tar -zxvf shadowsocks-libev-$SHADOWSOCKS_VER.tar.gz \
    && pushd shadowsocks-libev-$SHADOWSOCKS_VER \
    && ./configure --prefix=/usr && make && make install \
    && popd && ldconfig \
    && ln -sf /usr/local/lib/libmbedcrypto.so.0 /usr/lib64/libmbedcrypto.so.0 \
    && rm -rf shadowsocks-libev-$SHADOWSOCKS_VER shadowsocks-libev-$SHADOWSOCKS_VER.tar.gz
